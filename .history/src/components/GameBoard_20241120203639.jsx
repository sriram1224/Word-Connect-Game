...
const GameBoard = () => {
  ...
  useEffect(() => {
    ...
    // Retrieve highest score from localStorage
    ...
    if (selectedWords.length < groupSize && !selectedWords.includes(word)) {
      ...
      if (newSelection.length === groupSize) {
        ...
        if (isMatch) {
          ...
          setScore((prevScore) => {
            const newScore = prevScore + 10; // Add 10 points for each match
            // Update highest score in localStorage
            const savedScore = localStorage.getItem("highScore");
            if (!savedScore || newScore > parseInt(savedScore, 10)) {
  ...
  const shareMessage = `I scored the highest score of ${score} in the word count game! Come and play this game.`;
  ...
  return (
    <div className="bg-black min-h-screen flex flex-col items-center py-10">
      ...
      >
        {words.map((word, index) => (
          <div
            ...
            style={{
              backgroundColor: "#5b9aa0", // Custom color for background
              color: "#d6d4e0", // Custom text color
            }}
          >
            {word}
          </div>
      ...
      <div className="mt-10 flex flex-col items-center">
        ...
        <p className="text-blue-500 text-lg font-medium mt-4">Score: {score}</p>
      </div>
